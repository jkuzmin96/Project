...
function model_load($page)
{
    ...
    $query = 'SELECT ID, Nimetus, Kogus FROM jkuzmin__kaubad1 ORDER BY Nimetus ASC LIMIT ?,?';
...
function model_add($nimetus, $kogus)
{
    ...
    $query = 'INSERT INTO jkuzmin__kaubad1 (Nimetus, Kogus) VALUES (?, ?)';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
...
function model_delete($id)
{
    ...
    $query = 'DELETE FROM jkuzmin__kaubad1 WHERE ID=? LIMIT 1';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
...
function model_update($id, $kogus)
{
    ...
    $query = 'UPDATE jkuzmin__kaubad1 SET Kogus=? WHERE ID=? LIMIT 1';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
...
function model_user_add($kasutajanimi, $parool)
{
    ...
    $query = 'INSERT INTO jkuzmin_kasutajad (Kasutajanimi, Parool) VALUES (?, ?)';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
...
function model_user_get($kasutajanimi, $parool)
{
    ...
    $query = 'SELECT ID, Parool FROM jkuzmin_kasutajad WHERE Kasutajanimi=? LIMIT 1';
    $stmt = mysqli_prepare($l, $query);
    if (mysqli_error($l)) {
        echo mysqli_error($l);
        exit;
